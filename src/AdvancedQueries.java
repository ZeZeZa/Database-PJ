...
public class AdvancedQueries {
        ...
        if (duration == 0) {
            if (method == 0) {
                sql = "SELECT favorite_dish.dish_id,name, SUM(online*count )AS online FROM dish,orders,favorite_dish "
                        + "WHERE dish.id=orders.dish_id AND favorite_dish.customer_id=" + Integer.toString(customer_id)
                        + " AND dish.id=favorite_dish.dish_id AND orders.time>DATE_SUB(NOW(), INTERVAL 1 WEEK) GROUP BY dish_id";
            } else if (method == 1) {
            ...
            if (method == 0) {
                sql = "SELECT favorite_dish.dish_id,name, SUM(online*count )AS online FROM dish,orders,favorite_dish "
                        + "WHERE dish.id=orders.dish_id AND favorite_dish.customer_id=" + Integer.toString(customer_id)
                        + " AND dish.id=favorite_dish.dish_id AND orders.time>DATE_SUB(NOW(), INTERVAL 1 MONTH) GROUP BY dish_id";
            } else if (method == 1) {
                sql = "SELECT favorite_dish.dish_id,name, SUM((online=False)*count ) AS offline FROM dish,orders,favorite_dish "
                        + "WHERE dish.id=orders.dish_id AND favorite_dish.customer_id=" + Integer.toString(customer_id)
                        + " AND dish.id=favorite_dish.dish_id AND orders.time>DATE_SUB(NOW(), INTERVAL 1 MONTH) GROUP BY dish_id";
            ...
            if (method == 0) {
                sql = "SELECT favorite_dish.dish_id,name, SUM(online*count )AS online FROM dish,orders,favorite_dish "
                        + "WHERE dish.id=orders.dish_id AND favorite_dish.customer_id=" + Integer.toString(customer_id)
                        + " AND dish.id=favorite_dish.dish_id AND orders.time>DATE_SUB(NOW(), INTERVAL 1 YEAR) GROUP BY dish_id";
            } else if (method == 1) {
                sql = "SELECT favorite_dish.dish_id,name, SUM((online=False)*count ) AS offline FROM dish,orders,favorite_dish "
                        + "WHERE dish.id=orders.dish_id AND favorite_dish.customer_id=" + Integer.toString(customer_id)
                        + " AND dish.id=favorite_dish.dish_id AND orders.time>DATE_SUB(NOW(), INTERVAL 1 YEAR) GROUP BY dish_id";
    ...
    public void customerAnalysis(int customer_id, int interval) {
        ...
        if (interval == 0)
            sql = "SELECT time AS 时间段,count(*) AS 下单量"
                    + "FROM orders"
                    + "WHERE orders.customer_id=" + Integer.toString(customer_id)
                    + " AND orders.time>DATE_SUB(NOW(), INTERVAL 10 WEEK)"
                    + "GROUP BY UNIX_TIMESTAMP(timestamp) DIV 604800";
        else if (interval == 1)
            sql = "SELECT time AS 时间段,count(*) AS 下单量"
                    + "FROM orders"
                    + "WHERE orders.customer_id=" + Integer.toString(customer_id)
                    + " AND orders.time>DATE_SUB(NOW(), INTERVAL 10 MONTH)"
    ...
    public void dishSalesOfDedecatedCustomer(int threshold, int merchant_id) {
        String sql = "SELECT dish.id AS 菜品编号,dish.name AS 菜品名称,SUM(count) AS 忠实用户销量 "
                ...
                + "(SELECT customer_id FROM orders,dish WHERE orders.dish_id=dish.id AND dish.merchant_id ="
                + Integer.toString(merchant_id) + " GROUP BY orders.customer_id HAVING SUM(count)>"
    ...
    public void dishStatAnalysis(int merchant_id) {
        String sql = "SELECT dish.id AS 菜品编号,dish.name AS 菜品名称,SUM(count) AS 总销量,AVG(rating) AS 评分,"
                ...
                + "WHERE dish.id=orders.dish_id AND dish.merchant_id=" + merchant_id
                + " AND dish_review.dish_id=dish.id AND temp.dish_id=dish.id"
                + "GROUP BY dish.id";
    ...
    private ResultSet findCustomerMostDedicated(int dish_id) {
        String sql = "SELECT customer.id FROM customer,orders WHERE AND orders.customer_id=customer.id AND orders.dish_id="
                + Integer.toString(dish_id) + " GROUP BY customer.id ORDER BY 购买量 DESC LIMIT 1";
        try {
            ResultSet rs = statement.executeQuery(sql);
            if (!rs.isBeforeFirst()) {
                System.out.println("没有找到符合的记录");
